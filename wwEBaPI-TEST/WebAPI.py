...
treshold = 0.44
#    ///////00000000000000000000000 Read Dataset, input:Paths
Sequences = []
...
# print(PatternsArray)
# // Tokenised
strresult1 = [sentence.split() for sentence in strresult]
...
if Variant == 'RNN':
    loaded_CNNModel = Sequential()
    loaded_CNNModel = load_model('Offline/RNNModel.h5')
    Resultprediction = loaded_CNNModel.predict(x_train)

if Variant == 'LSTM':
    loaded_CNNModel = Sequential()
    loaded_CNNModel = load_model('Offline/LSTMModel.h5')
    Resultprediction = loaded_CNNModel.predict(x_train)

if Variant == 'GRU':
    loaded_CNNModel = Sequential()
    loaded_CNNModel = load_model('Offline/GRUModel.h5')
    Resultprediction = loaded_CNNModel.predict(x_train)

if Variant == 'DT':
    clf = load('Offline/DTmodel.joblib')
    Resultprediction = clf.predict(x_train)

if Variant == 'NB':
    clf = load('Offline/NBmodel.joblib')
    Resultprediction = clf.predict(x_train)
